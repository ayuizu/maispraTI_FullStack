...
-- Alunos Capacidade
SELECT P.professor_id, P.nome, T.turma_id AS Maior_Turma, T.qtd_alunos as Quantidade_Alunos FROM Professores P
	LEFT JOIN Turmas T ON P.professor_id = T.professor_id
    WHERE T.qtd_alunos = (
		SELECT MAX(T2.qtd_alunos) FROM Turmas T2 WHERE T2.professor_id = P.professor_id);
...
SELECT A.aluno_id, A.nome, M.numero_faltas, T.turma_id, T.tipo_curso FROM Alunos A
	INNER JOIN Matriculas M ON A.aluno_id = M.aluno_id
    INNER JOIN Turmas T ON M.turma_id = T.turma_id
    GROUP BY A.aluno_id, M.matricula_id, T.turma_id, T.tipo_curso
	...
	ORDER BY M.numero_faltas DESC;

-- 4.8) Listar a quantidade média de alunos por curso. 
SELECT tipo_curso, AVG(num_alunos_turma) AS Media_Num_Alunos FROM(
	-- Não dá apra usarAVG(COUNT(..)) >> subconsulta
	SELECT T.tipo_curso, COUNT(M.aluno_id) AS num_alunos_turma FROM Matriculas M
	INNER JOIN Turmas T ON M.turma_id = T.turma_id
    GROUP BY T.tipo_curso, T.turma_id
    ) AS AlunosPorTurma
    GROUP BY tipo_curso;
