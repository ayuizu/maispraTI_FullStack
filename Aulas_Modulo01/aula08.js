//*Exercícios sobre arrays e funções

//*Exercício 1: Somar todos os elementos de um Array - criar uma função

let elementos = [1, 2, 3, 4];
let array = [10, 20, 150, 200]; //Roda com 'array', pois a palavra reservada é 'Array', mas não é uma boa prática
...
//*Exercício 2: Encontrar o maior número num Array

let elementos2 = [1, 2, 3, 4];
...
console.log("O maior número do vetor é ", maiorElemento(elementos2));

//Resolução professor
function maiorNumero(vetor) {
  let maior = vetor[0]; //armazena posição zero do vetor para retornar algum elemento de qqr jeito
...
console.log(
  "Dentre os elementos, o maior é ",
  maiorNumero([1, 10, 600, 870, 45])
);
...
function reverterElementos(vetor) {
  let novoVetor = Array();
  for (let i = 0; i < vetor.length; i++) {
    //vetor.length = quantidade de elementos [0,1,2,3] = 4. Ultima posição = vetor.length - 1
    novoVetor[i] = vetor[vetor.length - 1 - i];
  }
  return novoVetor;
}
console.log("O array revertido é ", reverterElementos(elementos3));
...
console.log("O array revertido é ", reverterArray(elementos3));
